...
export default function IncomeModal({ income, onClose }: IncomeModalProps) {
    ...
    const fetchCategories = async () => {
        try {
            ...
            setCategories(data.filter((c: Category) => c.type === 'income'));
        } catch (error) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const url = income
                ? `${process.env.NEXT_PUBLIC_API_URL}/income/${income.id}`
        ...
        <div className="modal-overlay" onClick={onClose}>
            <div className="modal max-w-md" onClick={(e) => e.stopPropagation()}>
                <div className="modal-header">
                    <h2 className="modal-title">{income ? 'Edit Income' : 'Add Income'}</h2>
